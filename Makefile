...
NPM_PKG_NAME := @dhbw-cloud/dynamic-zones-client
...
.PHONY: all build clean doc convert client bundle npm-package npm-pack npm-publish check swag run help install-npm docker docker-login docker-build multi-arch-build dev helm-update bump version-check
...
npm-package: client
	...
	@VERSION=$$(cat VERSION | tr -d '\n'); printf '%s\n' \
		...
		'  "name": "$(NPM_PKG_NAME)",' \
		"  \"version\": \"$$VERSION\"," \
		'  "description": "Generated TypeScript client for the DHBW Cloud $(PROJECT_NAME) API.",' \
		'  "license": "Apache-2.0",' \
		'  "type": "module",' \
